...
class Dataset(object):
    ...
    # consecutives, for merging purpose
    MAX_SILENCE_DIFF = -1

    @staticmethod
    def clean_index(df):
        if "start" in df.columns:
            df = df.sort_values(by=["wave", "start"],
                                axis=0,
                                ascending=True,
                                ignore_index=True)
        else:
    ...
    def __init__(self, directory, config=None, corrections=None, vocab=None, repertoire=None):

        self.df = None
        self.config = Config(**default_config())
        self.corrections = {"syll": {}, "sample": {}}
        self.vocab = None
        ...
        if self.df is not None:
            if self.df.get("syll") is not None:
                ...
                raise ValueError("'vocab' can't be None if no annotations are available in the dataframe.")
        else:
            raise ValueError(f"{directory} is empty or no .csv file are in there. Can't build Dataset.")
    ...
    def _apply_corrections(self, df=None, corrections=None):
        ...
        if corrections is not None:
            ...
            if corrections.get("sample") is not None:
                ...
                print("'sample' entry in corrections not found, skipping sample correction.")

            # Delete samples
            df.at[df["syll"] == "DELETE", "syll"] = 'SIL'
    ...
    def _get_min_duration(self, config=None):
        ...
        try:
            ...
            n_fft, min_analysis_win = config.get("n_fft", "min_analysis_window_per_sample")
            return min_analysis_win * n_fft
        except Exception:
            return .0
    ...
    def _remove_short_samples(self, df=None, config=None):
        ...
        too_short = df[df["duration"] <= self._get_min_duration(config)]
        if len(too_short) > 0:
            print(f"{len(too_short)} annotations ignored - too short to be analysed"
                  f"(< {self._get_min_duration()*1000}ms) :"
                  f"{list(too_short.groupby('syll').groups.keys())}")

            df.at[too_short.index, "syll"] = "SIL"
    ...
    def _join_groups(self, df=None, config=None):
        ...
        for song in songs:
            labels = df[df['wave'] == song].copy()
            labels["start_d"] = labels["start"].shift(-1)
            labels["diff"] = labels["start_d"] - labels["end"]
            first_consecutives = labels[(labels["syll"].shift(-1) == labels["syll"])
                                        & ~(labels["syll"].isin(excluded_sylls))
                                        & (labels["diff"] > Dataset.MAX_SILENCE_DIFF)
                                        & (labels["diff"] != np.nan)]
            labels_c = labels.copy()
            for first in first_consecutives.itertuples():
                second = labels.index[first.Index + 1]
                labels_c.at[second, "start"] = first.start
                labels_c = labels_c.drop(first.Index, axis=0)

            new_dfs.append(labels_c.drop(["start_d", "diff"], axis=1))
    ...
    def _tag_silence(self, df=None):
        ...
        silence_samples = silence_samples.drop(["syll", "start", "end", "duration"], axis=1)
    ...
    def update(self, corrections=None, checkpoint_to=None,
               to_original=False):
        ...
        if corrections is not None:
            self.corrections = self.update_corrections(corrections)
    ...
    def update_corrections(self, news, olds=None):
        ...
        if corrections is not None:
            ...
            updated = {'syll': {k: v for k, v in old_syll.items() if k != v and v is not None}}

            updated["sample"] = {k: updated["syll"][v]
                                 if updated["syll"].get(v) is not None else v
                                 for k, v in old_sample.items()}

            return updated

    def refine(self, df=None, corrections=None, config=None):
        """
        Apply corrections to the dataset (class fusion, sample correction,
        very short samples removal, consecutive join)
        """
        df = self._apply_corrections(df, corrections)
        # df = self._remove_short_samples(df, config)
        # df = self._join_groups(df, config)
        # print(f"len(df) apres join: {len(df)}")
        df = self._tag_silence(df)

        return df

    def to_features(self, split=False, mode="syn", df=None, config=None,
                    max_songs=None, return_dict=False):
        """
        ...
        if split:
            train_songs, train_phrases, test_songs = self.to_trainset(df=df,
                                                                      config=config,
                                                                      max_songs=max_songs)
    ...
    def to_trainset(self, df=None, config=None, max_songs=None, test_ratio=None):
        """
        Returns split dataset for syn, non syn and ensemble training
        """

        train_songs, test_songs = self.split_syn(df=df, config=config, max_songs=max_songs, test_ratio=test_ratio)
    ...
    def balance_nonsyn(self, df=None, config=None):
        ...
        while(resampled.groupby("syll")["duration"].sum().min() < min_duration):
            resampled = all_s_rpz.sample(n=N, replace=True, weights=all_weights,
                                         random_state=rs)
        ...
        while(subsampled.groupby("syll")["duration"].sum().min() < min_duration):
            subsampled = all_o_rpz.sample(n=N, replace=True, weights=all_weights,
                                          random_state=rs)
            N += 5
    ...
    def split_syn(self, max_songs=None, df=None, config=None, test_ratio=None):
        ...
        syllables = self.vocab

        # Select the least represented syllables
        minus = df.groupby('syll')['syll'].count().index[df.groupby('syll')['syll'].count() <= 1]
        ...
        i = 1
        while len(reduced_syn.groupby('syll')) < len(syllables):
            i += 1
            minus = df.groupby('syll')['syll'].count().index[df.groupby('syll')['syll'].count() < i]
            minus_wave = df[df['syll'].isin(minus)].groupby('wave').count().index.values
            reduced_syn = df[df['wave'].isin(minus_wave)]
        ...
        if max_songs and max_songs < len(reduced_syn.groupby('wave')) and max_songs > -1:
            print(f"Warning : only {max_songs} will be selected.")

        already_picked = list(reduced_syn.groupby('wave')['wave'].groups.keys())
        left_to_pick = list(df[~df['wave'].isin(already_picked)].groupby('wave')['wave'].groups.keys())

        more_size = math.floor((1 - test_ratio) * len(left_to_pick) - test_ratio * len(already_picked))
        ...
        if max_songs:
            reduced_syn = self.sample_songs(max_songs, df=reduced_syn,
                                            config=config)

        print('Final repartition of data :')
        print(f"Train : {len(reduced_syn.groupby('wave'))}, Test: {len(test_syn.groupby('wave'))}")
...
class Config(dict):
    ...
    def frames(self, duration):
        ...
        """
        return round(round(duration * self["sampling_rate"]) / self.as_fftwindow("hop_length"))

    def to_duration(self, mfcc_frames):
        return (mfcc_frames * self["hop_length"]) / self["sampling_rate"]

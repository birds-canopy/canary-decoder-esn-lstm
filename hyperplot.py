...
def _outliers_idx(values, max_deviation):
    ...
    dist = abs(values - minimum)
    corrected = dist < max_deviation
    return corrected
...
def _logscale_plot(ax, xrange, yrange, base=10):
    ...
    if xrange is not None:
        ...
        ax.set_xlim([xrange.min() - 0.05*xrange.min(),
                     xrange.max() + 0.05*xrange.min()])
    if yrange is not None:
        ax.yaxis.set_minor_formatter(ticker.LogFormatter())
        ax.yaxis.set_major_formatter(ticker.LogFormatter())
        ax.set_yscale("log", base=base)
        ax.set_ylim([yrange.min(),
                     yrange.max()])

    ax.margins(x=0.01, y=0.01)


def _scale(x, score_neg_behaviour=None):
    # TODO: find a better normalization
    if score_neg_behaviour == "zero":
        s = x.copy()
        s[s < 0] = 1e-1
        return (s - s.min()) / (s.ptp())
...
def _cross_parameter_plot(ax, values, scores, loss, smaxs, cmaxs,
    ...
    sc_l = ax.scatter(X[lmaxs], Y[lmaxs], scores[lmaxs]*100,
                      c=loss[lmaxs], norm=LogNorm(), cmap="BuPu_r")
    sc_s = ax.scatter(X[smaxs], Y[smaxs], scores[smaxs]*100,
                      c=cmaxs, cmap="autumn_r")
...
def _loss_plot(ax, values, scores, loss, smaxs, cmaxs, lmaxs,
    ...
    sc_l = ax.scatter(X[lmaxs], loss[lmaxs], scores[lmaxs]*100, color="forestgreen", alpha=0.3)
    sc_s = ax.scatter(X[smaxs], loss[smaxs], scores[smaxs]*100, c=cmaxs, cmap="autumn_r")
    if loss_behaviour == "min":
        sc_m = ax.scatter(X[~(lmaxs)], [loss.min()]*np.sum(~lmaxs),
                          scores[~(lmaxs)]*100, color="red", marker="*", label="Loss min.")
    else:
        sc_m = ax.scatter(X[~(lmaxs)], [loss.max()]*np.sum(~lmaxs),
                          scores[~(lmaxs)]*100, color="red", marker="*", label="Score max.")
...
def _parameter_violin(ax, values, scores, loss,
    ...
    ax.scatter(np.log10(values[p][loss.argmin()]), 1, color="red",
               zorder=5, label="Best loss", marker="*")

    if log:
        ax.xaxis.set_major_formatter(mticker.StrMethodFormatter("$10^{{{x:.0f}}}$"))
        min_exp = round(np.floor(all_y.min()))
        max_exp = round(np.ceil(all_y.max()))
        ax.xaxis.set_ticks([np.log10(x) for p in range(min_exp, max_exp) for x in np.linspace(10**p, 10**(p+1), 10)], minor=True)
...
def plot_hyperopt_report(exp, params, metric='loss', loss_metric="loss",
                         ...
                         percentage=0.1, score_neg_behaviour="zero"):
    ...
    scores = _scale(scores, score_neg_behaviour)
    ...
    percent = math.ceil(len(scores) * percentage)
        ...
        for j, p2 in enumerate(params):
            ...
            if p1 == p2:
                ...
                sc_l, sc_s, sc_m = _cross_parameter_plot(ax, values,
                                                         ...
                                                         p1 in categorical,
                                                         p2 in categorical)
                last_crossparam = sc_l

    # legends
    # handles, labels = sc_l.legend_elements(prop="colors")
    ticks = [loss.min(), (loss.max() - loss.min()) / 2, loss.max()]
    l_cbar = plt.colorbar(last_crossparam, ticks=ticks, cax=lbar_ax, ax=axes, orientation="horizontal")
    _ = l_cbar.ax.set_title(f"Loss value (min: {round(loss.min(), 6)})")
    ...
    f_cbar = fig.colorbar(sc_s, cax=fbar_ax,
                          orientation="horizontal", ticks=[0, 0.5, 1])
    _ = f_cbar.ax.set_title(f"{metric} best population")
    _ = f_cbar.ax.set_xticklabels([f"{round(percentage*100)}% best",
                                   f"{round(percentage*100/2)}% best",
                                   "Best"])
    ...
    for i, p in enumerate(params):
        ...
        if i == 0:
            ax.set_ylabel(f"{round(percentage*100)}% best {metric}\nparameter distribution")
    return fig

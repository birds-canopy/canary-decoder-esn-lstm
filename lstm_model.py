...
REPORT = Path("reports/marron1/lstm-2")
...
def load_data_for_lstm(files):
    xs, ys = [], []
    for s in files:
        data = np.load(PROCESSED_DATA / s)
        xs.append(data["x"])
        ys.append(data["y"])
    xs = pad_sequences(xs, maxlen=MAX_SEQ_LEN)
    ys = pad_sequences(ys, maxlen=MAX_SEQ_LEN)
    return xs, ys
...
def cross_validation():
    ...
    for fold, (train_idx, val_idx) in enumerate(folds.split(files)):
        ...
        X, y = load_data_for_lstm(train_files)
        Xval, yval = load_data_for_lstm(val_files)
        ...
        for instance in range(5):
            ...
            tensorboard = K.callbacks.TensorBoard(log_dir=f'logs-3/fold-{fold}-{instance}',
                                                  update_freq=10)
...
def reduced_songs_training():
    ...
    for max_song in [5, 10, 20, 30, 50, 70]:
        ...
        for instance, (train_idx, _) in enumerate(folds.split(train_files)):
            ...
            X, y = load_data_for_lstm(fold_files)
            Xval, yval = load_data_for_lstm(val_files)
